...
# ╔═╡ 8f9a1ec4-a3ea-11ec-3902-7155986e92d1
using Plots, VoronoiFVM, ExtendableGrids, PlutoVista, GridVisualize, PlutoUI

# ╔═╡ 0bf052da-cac7-4c55-bdcf-dce42c7c9aa7
default_plotter!(PlutoVista);

# ╔═╡ 41b4a0c4-7cdc-4082-b850-caf15a6a8af1
begin
	beta = 1.0
	gamma = 0.5
	epsilon = 0.1
...
# ╔═╡ 86f62068-b71c-4813-bc16-2665da6e7e2b
f_r(u,v) = u - (1/3)*u^3 - v

# ╔═╡ f3e633db-26e9-44d2-88d9-4fb6c9501dba
g(u,v) = u + beta - gamma*v

# ╔═╡ 562312d6-e613-4ca1-8e7e-7803987882b7
function get_initial_values(xgrid)
	function u_0(x)
		u = (0 ≤ x ≤ L/20) ? 2 : -1.28791
		return [u, 0, -0.57582]
	end
	return map(u_0, xgrid)
end

# ╔═╡ 8380d6c4-968e-407c-ba62-5ef64e292f5d
...
# ╔═╡ ea2236e1-ed03-4eba-812f-53731cf7d622
function reaction(f, u, node)
	f[1] = -f_r(u[1], u[3])/epsilon
	f[2] = 0
	f[3] = -epsilon*g(u[1], u[3])
end

# ╔═╡ 5eb04d10-bc3b-425c-833c-3813b95c0faf
function flux(f, u, edge)
	f[1] = sigma_i*(u[1,2]-u[1,1]) + sigma_i*(u[2,2] - u[2,1])
	f[2] = sigma_i*(u[1,2]-u[1,1]) + (sigma_i+sigma_e)*(u[2,2] - u[2,1])
	f[3] = 0
end

# ╔═╡ b07d6b82-34a3-4ed4-b627-5534b93e30cc
function bcondition(f, u, bnode)
	boundary_dirichlet!(f,u,bnode,species=1,region=1,value=0)
	boundary_dirichlet!(f,u,bnode,species=1,region=2,value=0)
...
# ╔═╡ ddaec33b-17fc-4ec0-afbd-018e4e9018ea
grid=simplexgrid(0:0.5:L)

# ╔═╡ 29ccf9dd-9d6f-4093-9ff0-904aa37652bd
system=VoronoiFVM.System(grid; flux,reaction,storage,bcondition,species=[1,2,3])

# ╔═╡ d487b586-c5dc-4bf6-84a6-466009589c04
inival = get_initial_values(grid)

# ╔═╡ 559c2059-cd28-4ba6-b0b0-379a73dcfb6a
init = unknowns(system)

# ╔═╡ c87e0580-4491-4b8a-9544-fe68b16c907b
init .= [tuple[k] for tuple in inival, k in 1:3]'

# ╔═╡ ad82850e-b126-4ac3-a845-767b24912a79
tsol=solve(system;init,times=(0,T),Δu_opt=0.01,Δt_min=1.0e-2, Δt=1.0e-2)

# ╔═╡ 73e40e85-5288-487d-93e7-bad42114ccb9
md"""
t= $(@bind t_plot Slider(0:T/100:T,default=0))
"""

# ╔═╡ b331afa5-59ab-4ff2-8883-cb5614387ff9
sol = tsol(t_plot)

# ╔═╡ 29cfae7d-934c-41e8-b42e-3edd9f3c6768
vis=(GridVisualizer(;size=(200,200),dim=1), GridVisualizer(;size=(200,200),dim=1), GridVisualizer(;size=(200,200),dim=1))

# ╔═╡ a1806914-f2fa-4582-ba01-9ea0451d6a78
scalarplot!(vis[1],grid,sol[1,:],limits=(0,5),show=true,colormap=:summer)

# ╔═╡ 850a0409-3a9d-41ed-86bb-49a7b8785b96
scalarplot!(vis[2],grid,sol[2,:],limits=(0,5),show=true,colormap=:summer)

# ╔═╡ 3653676e-9f45-47c4-a3d5-b9ffec6c613f
scalarplot!(vis[3],grid,sol[3,:],limits=(0,5),show=true,colormap=:summer)
