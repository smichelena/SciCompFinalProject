...
# ╔═╡ 571006de-2e62-4d1a-ab64-7b39cff790b8
begin
	using ShortCodes, ExtendableGrids, VoronoiFVM
	using Plots, GridVisualize, PlutoUI
...
md"""
# Bidomain problem
The bidomain problem is a system of partial differential equations simulating the electrical activity of the heart
"""

# ╔═╡ 0ca0214d-55c8-4e72-a0eb-2fc8708adf25
refs= Dict([
	("eth_bou", (1, DOI("10.1137/070680503"))),
]);

# ╔═╡ c347a948-17b8-44c6-9a7f-3683363ec3b7
function cite(name)
	return string("[",refs[name][1],"]")
end

# ╔═╡ b216bde4-6cd9-468c-b283-13a7fa554bcb
cite("eth_bou")

# ╔═╡ 9653fb91-a348-4c4e-9892-020211969393
md"""
## PDE
The Bidomain problem for membrane-based models is given by Ethier & Bourgalt as
```math
...
# ╔═╡ a0c738b6-9e0d-4ea7-86ac-7fb9c33bf731
begin
	function u⃗₀(x)
		u₀, v₀ = fg_zeros()
		if 0 ≤ x ≤ L/20
...
physics = VoronoiFVM.Physics(
	flux = function(y,u,edge)
		y[1] = -σᵢ*(u[1,1]-u[1,2]) - σᵢ*(u[2,1]-u[2,2])
		y[2] = σᵢ*(u[1,1]-u[1,2]) + (σᵢ+σₑ)*(u[2,1]-u[2,2])
		y[3] = 0
	end,
...
"""
function bidomain(;N=100, dim=1, Δt=1e-4, tₑ=T)
	grid = create_grid(N, dim)

	function bc(y,u,node)
		boundary_dirichlet!(y,u[2],node; region=1, value=0)
	end
	...
	control.Δt = Δt
	control.Δt_max = 0.1*tₑ
	control.Δu_opt = 0.05

	sol = solve(sys, inival=init, times=[0,tₑ]; control=control)
	return grid,sol
end
...
begin
	dim = 1
	N = 50
	grid, sol = bidomain(N=N, dim=dim)
	vis = GridVisualizer(resolution=(400,300), dim=dim)
end;

# ╔═╡ cac7537c-0d85-46e0-8cb9-7d6954ca061f
md"""
tₛ = $(@bind tₛ Slider(1:length(sol), default=1, show_value=true))
"""
...
begin
	...
	scalarplot!(vis, grid, sol[3,:,tₛ], linestyle=:dot, show=true)
end
