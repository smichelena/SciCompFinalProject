...
function initial_cond(sys, xgrid, Tinit_solve, dim, initial2D, use_csv, anisotropic)
	init = unknowns(sys)
	U = unknowns(sys)
...
function bidomain(;N=100, dim=1, Δt=1e-3, T=30, Plotter=Plots, 
	...
	init = initial_cond(sys, xgrid, Tinit_solve, dim, initial2D, use_csv, anisotropic)
	U = unknowns(sys)
...
let
	t = 3; spec = 2
	contour_2d_at_t(spec,t,Δt₂,xgrid₂,sol₂, 
...
let
	...
	contour_2d_at_t(spec,t,Δt₄,xgrid₄,sol₄, 
		...
		for "*species[spec]*" at t="*string(t))
end
...
# ╔═╡ 8e0d9d7b-77c9-4e56-bf19-3afb0fbefb47
images_for_gif(3, "contour_plot_2D_species_anisotropic", xgrid₄, sol₄; steps=5)
